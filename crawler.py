...
class GoogleMaps:
    ...
    def save_csv_file(self,scrapped_data,keyword):
        try:
            ...
            fieldnames = ["Name", "Phone", "Location"]
            ...
            print(f"Crawler Error: Something went wrong in file creation. Error: ",str(e))
    ...
    def google_map_crawler(self):
        try:
            ...
            for keyword in keywords:
                print(f"Crawler Logs: Crawling Google Maps for keyword: {keyword}.")
                try:
                ...
                try:
                    self.driver.find_element(By.XPATH,'//*[@id="searchboxinput"]').send_keys(keyword)
                    self.driver.find_element(By.XPATH,'//*[@id="searchbox-searchbutton"]').click()
                    time.sleep(5)

                    results =  self.driver.find_elements(By.XPATH, "//a[@class='hfpxzc']")
                    break_condition = False
                    focus_element =  self.driver.find_element(By.ID, 'zero-input')
                    while not break_condition:
                        temp = results[-1]
                        self.actionChains.scroll_to_element(results[-1]).perform()
                        self.actionChains.move_to_element(focus_element).click().perform()
                        for i in range(3):
                            self.actionChains.send_keys(Keys.ARROW_DOWN).perform()
                            time.sleep(2)
                        self.wait_for_element_location_to_be_stable(temp)

                        results = self.wait.until(EC.presence_of_all_elements_located((By.XPATH, "//a[@class='hfpxzc']")))
                        if results[-1] == temp:
    ...
    def google_map_scrapper(self,html,keyword):
        ...
        for i in results:
            try:
                ...
                inner_link_html = self.google_map_inner_link_scrap(inner_link)
                inner_link_html = BeautifulSoup(inner_link_html, "lxml")
                name = inner_link_html.find('h1','DUwDvf lfPIob').text
                print(f"Crawler Logs: Company: {name} scrapped successfully.")

                try:
                    location = inner_link_html.find('div','Io6YTe fontBodyMedium kR99db').text
                except:
                    location =  "Not Available"
                try:
                    phone = inner_link_html.find_all('div','Io6YTe fontBodyMedium kR99db')[1].text
                    phone = phone.replace(" ","")
